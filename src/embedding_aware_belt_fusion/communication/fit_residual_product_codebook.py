"""Fit a progressive residual codebook to frozen TrackFormer messages.

The first stage provides a coarse object message.  Every following stage
quantizes the remaining reconstruction residual, so a receiver can decode a
prefix of stages at a lower transmission rate.
"""
...
from embedding_aware_belt_fusion.communication.fit_product_codebook import (
    _load_model,
    _reservoir_update,
)


def _stage_specs(value: str) -> list[tuple[int, int]]:
    """Parse e.g. ``4x256,4x256,8x256`` into codebook stages."""
    stages = []
    for stage in value.split(","):
        try:
            groups, entries = stage.strip().lower().split("x", maxsplit=1)
            stages.append((int(groups), int(entries)))
        except ValueError as error:
            raise argparse.ArgumentTypeError(
                "--stages must look like 4x256,4x256,8x256"
            ) from error
    if not stages:
        raise argparse.ArgumentTypeError("--stages must not be empty")
    return stages
...
def parse_args():
    ...
    parser.add_argument(
        "--stages",
        type=_stage_specs,
        default=_stage_specs("4x256,4x256,8x256"),
        help="Progressive stages as groups x entries; default gives 32/64/128 bits.",
    )
...
def main():
    ...
    for groups, entries in args.stages:
        if dimension % groups:
            raise ValueError(
                f"Message dimension {dimension} is not divisible by {groups} groups"
            )
        if args.max_vectors < entries:
            raise ValueError("--max-vectors must be at least every stage's entries")
    ...
    with torch.no_grad():
        for frames in tqdm(loader, desc=f"collecting {args.representation} messages"):
            ...
            for source in sources:
                ...
                if args.representation == "query-state":
                    scores = output["pred_logits"][0].softmax(-1)[:, 0]
                    values = output["hs_embed"][0][
                        scores >= args.track_query_score_threshold
                    ]
                else:
    ...
    if retained < max(entries for _, entries in args.stages):
        raise RuntimeError("Not enough source messages to initialize every stage")
    quantizer = ResidualProductCodebook(dimension, args.stages).to(device)
    metrics = quantizer.fit(
        reservoir[:retained],
        iterations=args.kmeans_iterations,
        chunk_size=args.kmeans_chunk_size,
        seed=args.seed,
    )
    result = residual_checkpoint_payload(
        quantizer,
    ...
    print(
        "RESIDUAL_CODEBOOK "
        + json.dumps({key: value for key, value in result.items() if key != "state_dict"}, sort_keys=True)
    )
